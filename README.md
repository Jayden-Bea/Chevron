...
Chevron now prioritizes download throughput first: yt-dlp starts with an Android client strategy (commonly less throttled on YouTube), prefers adaptive video+audio formats before progressive fallbacks, and uses concurrent fragment fetches when the source supports segmented delivery. Ingest downloads to `workdir/source/` first and only then prepares `workdir/proxy.mp4`, so network transfer is no longer tied to immediate proxy conversion.
